...
replacements = {
    ...
    'test_21_no_lookahead_swing_k': '''    def test_21_no_lookahead_swing_k(self):
        ...
        df.loc[5, 'high'] = 110.0
        res = engine.analyze(df)
        # 110 is the peak. But the swing zone is created around it.
        peak_zone_low = 110.0 - 0.2 * res['ATRr'].iloc[5]
        self.assertTrue(res['NEAREST_RESISTANCE'].iloc[5] != peak_zone_low)
        self.assertTrue(res['NEAREST_RESISTANCE'].iloc[6] != peak_zone_low)
        self.assertTrue(res['NEAREST_RESISTANCE'].iloc[7] != peak_zone_low)
        self.assertEqual(res['NEAREST_RESISTANCE'].iloc[8], peak_zone_low, "Swing must be confirmed exactly at t+3")'''
}
